...
class WriteTab:
    ...
    def setup_ui(self):
        ...
        self.date_label = ttk.Label(
            self.frame, text=self.current_date, style="Date.TLabel"
        )
    ...
    def save_entry(self):
        ...
        if not content:
            messagebox.showwarning(
                "Empty Entry", "Please write something before saving."
            )
            return False

        try:
            if self.storage_manager.save_entry(
                content, self.mood_var.get(), self.current_date
            ):
                self.text_area.delete("1.0", tk.END)
                messagebox.showinfo(
                    "Saved", "Your diary entry has been securely saved!"
                )
                ...
                self.status_callback("Error saving entry")
                return False
